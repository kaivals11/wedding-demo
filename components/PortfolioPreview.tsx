...
export default function PortfolioPreview() {
  ...
  // The 'Fast' images will move further up the Y-axis (-250px) than the 'Slow' images (-100px)
  ...
  return (
    <section ref={containerRef} className="relative w-full py-24 md:py-48 bg-[#F7F5F0] text-[#2A2A2A] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-4 relative">
        
        {/* Image 1: Large Base Landscape (Moves slowly) */}
        <motion.div 
          style={{ y: ySlow }}
          className="md:col-span-7 md:col-start-1 group cursor-pointer z-0"
        >
        ...
        {/* Image 2: Vertical Portrait overlapping Image 1 (Moves incredibly fast) */}
        <motion.div 
          style={{ y: yFast }}
          className="md:col-span-4 md:col-start-8 md:-mt-48 z-10 group cursor-pointer hidden md:block"
        >
       ...
       {/* Image 3: Abstract Square sitting below (Moves slowly) */}
        <motion.div 
          style={{ y: ySlow }}
          className="md:col-span-5 md:col-start-4 md:mt-32 z-0 group cursor-pointer"
        >
